...
const Item : React.FC<ItemProps> = ({ color, color2, setColor, itemColor, reactItemColor }) => {
    function clickColor(){
    ...
    return (
        <>
            <div className={`aspect-square w-6 sm:w-12 ${reactItemColor} rounded-md sm:rounded-xl ${(color2 === itemColor) ? "border-3" : "border-2"} border-black`} onClick={clickColor}>
                <img className="p-0.5 sm:p-2" src={paintBrushURL}/>
            </div>
        </>
    )
};
    ...
    return (
        <>
            <div className="flex space-x-2">
                <Item color={color} color2={color2} setColor={setColor} itemColor="#FFFFFF" reactItemColor="bg-white hover:bg-slate-200"/>
